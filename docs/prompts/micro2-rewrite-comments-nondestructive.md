...
Ask the model to rewrite something without altering meaning or inserting new content.

You will write a prompt that:
- rewrites PL/SQL comments for clarity
- must not alter logic
- must not add business rules
- must ask for clarifying questions if anything is unclear
- must preserve variable names, indentation, and formatting
